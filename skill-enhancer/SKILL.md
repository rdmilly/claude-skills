...
**Step 2.3 — Multi-run deduplication (when running multiple lenses)**

If running multiple lens passes (e.g., 3 runs with different lenses), deduplicate BEFORE the quality gate. Merge candidates that express the same idea from different angles into a single, strongest version. In a 3-run test on Revenue Architecture, 2 of 33 candidates were duplicates — identical ideas surfaced by different lenses.

**Step 2.4 — Diminishing returns check**

After each lens run, track the cut rate at the quality gate. If a run produces >30% cuts (more than 30% of its candidates fail the quality gate), stop running additional lenses — you're past the point of useful expansion. In the 3-run test: Run 1 cut 1/12 (8%), Run 2 cut 2/11 (18%), Run 3 cut 3/10 (30%). Run 3 was at the boundary — a 4th run would likely have produced mostly waste.
...
│  "Was this applied in last 3-5 uses? (or: would it be applied?)"      │
...
The 30% length cap is the hard constraint for preventing runaway growth. But length alone is the wrong metric — a skill that's 25% longer but 50% more useful is a win. Track **value-density ratio**: (number of quality-gate-passing elements) / (total token count). If the ratio stays flat or improves, the enhancement is working. If the ratio drops (you added tokens but not proportional value), the expansion was too loose.

In the Revenue Architecture 3-run test, the v2 was actually 2% SHORTER than v1 (9,097 vs 9,333 bytes) because aggressive pruning and compression offset the additions. The value-density ratio improved — same length, more useful output per token.
...
| **Cross-lens duplication** | Same idea generated by multiple lenses with different wording | Merge into single strongest version before quality gate. Deduplicate between runs, not after. |
